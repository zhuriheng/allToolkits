...
def net_init():
    ...
    sym_file = "/tmp/eval/init/deploy.symbol.json"
    params_file  = "/tmp/eval/init/weight.params"
    label_file = "/tmp/eval/init/labels.csv"
    ...
    update_config("/tmp/eval/init/resnet.yaml")
...
def vis(imageName=None,bbox_list=None):
    ...
    for bbox_dict in bbox_list:
        ...
        cv2.putText(im, '%s %.3f' % (cls_name, score), (bbox[0][0], bbox[0][1] + 15), color=color_black, fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5)
...
def main():
    ...
    model = net_init()
    _t2 = time.time()
    print("net init time : %f"%(_t2-_t1))
    _t1 = time.time()
    res = net_inference(model)
    _t2 = time.time()
    print("net_inference time : %f" % (_t2-_t1))
    classes = model['classes']
