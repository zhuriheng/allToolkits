...
def parse_args():
    ...
    parser.add_argument('--visualize', help='visualize the detect reuslt', type=bool,default=False)
    parser.add_argument('--visualizeOutPutPath',help='visualizeOutPutPath', type=str)
...
def show_boxes_write_rg(fileOp=None, image_name=None, im=None, dets=None, classes=None,vis=False, scale=1.0):
    ...
    fileOp.flush()
    if vis:
        out_file = os.path.join(args.visualizeOutPutPath, imageName.split('/')[-1])
...
def show_boxes(isUrlFlag=None, im_name=None, dets=None, classes=None, scale=1, vis=False, fileOp=None, flag=1):
    ...
    if flag == 1:
        show_boxes_write_rg(fileOp=fileOp,image_name=im_name,im=im,dets=dets,classes=classes,vis=vis)
    elif flag == 2:
...
def init_detect_model():
    ...
    arg_params, aux_params = load_param(cur_path + '/demo/models/' + ('rfcn_voc'), int(args.epoch), process=True)
...
def process_image_fun(isUrlFlag=False, imagesPath=None, fileOp=None,vis=False):
    ...
    endCount = beginCount
    for i in range((len(imagesPath)-beginCount)/ONE_BATCH_IMAGE_COUNT):
...
def main():
    ...
    fileOp = open(args.outputFilePath,'a+') # 追加的方式，如果不存在就创建
...
--epoch 14 \
--localImageListFile  /workspace/data/BK/terror-dataSet-Dir/TERROR-DETECT-V0.9/ImageSets/Main/test.txt \
--localImageBasePath  /workspace/data/BK/terror-dataSet-Dir/TERROR-DETECT-V0.9/JPEGImages \
--outputFileFlag 1 \
--outputFilePath  /workspace/data/terror-det-Dir/rfcn-Dirs/rfcn-res101-v0.9-t1/TERROR-DETECT-V0.9-test-rf-output.txt \
--gpuId 0 \
> /workspace/data/terror-det-Dir/rfcn-Dirs/rfcn-res101-v0.9-t1/TERROR-DETECT-V0.9-test-rf-output.log 2> &1 &


nohup python -u terror-process-labex-rg-demo.py \
--configYamlFile resnet_v1_152_terror_dcn_rfcn_end2end_ohem.yaml \
--epoch 12 \
--localImageListFile  /workspace/data/BK/terror-dataSet-Dir/TERROR-DETECT-V0.9/ImageSets/Main/test.txt \
--localImageBasePath  /workspace/data/BK/terror-dataSet-Dir/TERROR-DETECT-V0.9/JPEGImages \
--outputFileFlag 1 \
--outputFilePath  /workspace/data/terror-det-Dir/rfcn-Dirs/rfcn-res152-v0.9-t3/TERROR-DETECT-V0.9-test-rf-output.txt \
--gpuId 0 \
> /workspace/data/terror-det-Dir/rfcn-Dirs/rfcn-res152-v0.9-t3/TERROR-DETECT-V0.9-test-rf-output.log 2>&1 &
"""
