...
def visualize_Fun(origim, bbox_list):
    ...
    for bbox_dict in bbox_list:
        ...
        cv2.putText(im, '%s %.3f' % (cls_name, score),
                    (bbox[0][0], bbox[0][1] + 15), color=color_black, fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5)
    return im
def processVideo(refinede_coco_class, refinede_face_class, videoPath=None):
    ...
    if '.' in os.path.basename(videoPath):
        output_video_path = videoPath[:videoPath.rfind('.')]+'-detect.mp4'
        write_file = videoPath[:videoPath.rfind('.')]+'-detect.json'
    else:
        write_file = videoPath+'-detect.json'
...
def main():
    ...
    processVideo(
        refinede_coco_class, refinede_face_class, videoPath="/workspace/data/BK/process-mp4/data/car2.mp4")
    pass
